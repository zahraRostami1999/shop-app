...
const DisplaycartItems: React.FC = () => {
  ...
  return cartItems.map((item: CartItem) => {
    ...
    if (matchingProduct) {
      return (
        <li key={matchingProduct.id} className="first:mt-20 flex flex-wrap justify-center items-center lg:w-11/12 md:w-11/12 w-full border-2 shadow-lg lg:px-10 md:px-8 px-2 py-3 lg:mb-10 md:mb-8 mb-5 rounded-lg">
          ...
          <div className="grid grid-cols-5 lg:grid-cols-5 gap-4 w-full lg:w-10/12 md:w-11/12 mt-4 lg:mt-0">
            ...
            <div className="col-span-1 flex justify-center items-center h-10 rounded-3xl ">
                <div className="w-1/6" onClick={() => handleAdd(matchingProduct.id, matchingProduct.price)}><OrangeBtn content="+" /></div>
                <div className="w-1/6 flex justify-center"><p className="text-sm font-semibold  text-left">{item.qty}</p></div>
                <div className="w-1/6" onClick={() => handleRemove(matchingProduct.id, item.qty, matchingProduct.title)}><GrayBtn content="-" /></div>
            </div>
