...
function CartPage() {
  ...
  const displayCartItems = () => {
    return cartItem.map((item) => {
      ...
      if (matchingProduct) {
        return (
          <li key={matchingProduct.id} className=" rounded my-9 mx-6">
            <div className="flex items-center">
              <img
                className="w-32 h-28 bg-cover "
                src={matchingProduct.image}
              />

              <div className="grid grid-cols-2 grid-rows-4 ml-6">
                <h2 className="row-start-1 font-bold">
  ...
  return (
    <>
      ...
      <div className=" border h-full shadow-lg mt-28 mb-10 mx-6 rounded-md">
        <ul>
          {cartItem.length > 0 ? displayCartItems() : <h1 className="text-center py-6 " >Cart is Empty</h1>}
        </ul>
      </div>
    </>
  );
